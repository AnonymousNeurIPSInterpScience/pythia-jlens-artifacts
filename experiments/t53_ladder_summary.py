...
def main() -> int:
    ...
    for scale, n_layers in (("410m", 24), ("1b", 16)):
        # floor, not round: cv6_per_family_ladder.band_for() and trainval.py both use int(),
        # and at 12 layers round() disagrees ([5,9] against the [4,9] that was actually fitted).
        # A no-op for the two scales this function evaluates (410M, 1B), correct at 160M.
    ...
    rec = {
        ...
        "inputs": [os.path.relpath(L410, os.path.join(HERE, "..")) + "/*.json",
                   ...
                   "results/lenses/e28/e28_*_410m_n*_s*.pt (filenames only)",
