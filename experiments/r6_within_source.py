...
PRE-REGISTRATION: docs/experiments/preregs/R6_within_source_resampling.md, committed before this ran.
Source slate: POSTREVIEW_EXPERIMENTS.md §3 Tier B, item R6.
...
def main() -> int:
    ...
    if obs is None:
        ...
        verdict = (f"REJECT — the effect is sample noise, not source identity. The between-source "
                   f"share is {obs:.4f} <= 0.30. FLAGGED FOR ADJUDICATION: the paper's factor is "
    ...
    prereg = "docs/experiments/preregs/R6_within_source_resampling.md"
