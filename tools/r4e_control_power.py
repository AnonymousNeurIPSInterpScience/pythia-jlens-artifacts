...
PRE-REGISTRATION: docs/experiments/preregs/R4_corrections.md item R4e, committed before this script existed.
Source slate: POSTREVIEW_EXPERIMENTS.md §3 Tier A, item R4e.
...
WHAT IS MECHANICAL AND WHAT IS JUDGEMENT, kept apart on purpose:
  ...
  MECHANICAL (this script decides, and can be re-run):
    ...
    * DECLARED_NEVER_CODED -- a control named in a docs/experiments/preregs/ document or a prereg that has
...
def main() -> int:
    ...
    declared_missing = []
    spec_files = glob.glob(R("docs", "experiments", "preregs", "*.md")) + glob.glob(R("docs", "archive", "prereg", "*.md"))
    stem_to_results = {}
    for rel in per_file:
    ...
    rec = {
        ...
        "REQUIRED_CHANGE_TO_THE_SPEC_TEMPLATE": (
            "docs/experiments/README.md's CONTROLS row becomes: 'each control, the number it "
