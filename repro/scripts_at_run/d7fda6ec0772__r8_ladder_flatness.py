...
PRE-REGISTRATION: specs/experiments/R8_ladder_rstrip_S2.md, committed before the flag existed.
...
def main() -> int:
    ...
    ctrl["C4_N_grid_identical_across_seeds"] = {
        ...
        "why_it_matters": (
            ...
            "missing its N=800 block (results/e28_Wikipedia_en_410m_n800_s2.pt does not exist), and "
    ...
    if not C1:
        ctrl["C1_recovery_without_the_flag"] = {
            ...
            "checked": False, "c1_dir": a.c1_dir,
            "fires": False,
            "why_not": (f"no re-scored cells found in {a.c1_dir}. Produce them with: "
                        f"experiments/e28_ladder_410m.py --device cpu --cells 'Github:0' "
                        f"--out {a.c1_dir}  (no --rstrip). A control that did not run does not fire.")}
    else:
        ...
        ctrl["C1_recovery_without_the_flag"] = {
            "required": "the same code without --rstrip reproduces results/ladder410_cpu "
                        "at max_abs_diff = 0.0 over every (N, eval set, aggregation) value",
            "checked": True, "c1_dir": a.c1_dir, "cells_compared": cells,
    ...
    prereg = "specs/experiments/R8_ladder_rstrip_S2.md"
