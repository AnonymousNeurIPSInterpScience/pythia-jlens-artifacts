...
def main() -> int:
    out_path = os.path.join(RES, "e53_ladder_summary.json")
    ...
    for c in CORPORA:
        for s in (0, 1, 2):
            ...
            lad = json.load(open(os.path.join(RES, "ladder410", f"ladder_{c}_s{s}.json")))
    ...
    for scale, n_layers in (("410m", 24), ("1b", 16)):
        ...
        if scale == "410m":
            used = json.load(open(os.path.join(RES, "ladder410", "ladder_Github_s0.json")))["band"]
        else:
